function ApiCall() {
    ...
    const imageUrl = `https://mcapi.us/server/image?ip=${serverIp}`;
        ...
        .then(data => {
            ...
            {
            document.getElementById("checkmark").innerText = "online"
            ...
            if(!Array.isArray(data.motd_json.extra))
            {
                ...
                document.getElementById("motd").appendChild( parseStyle(data.motd_json));
            }else
            {
                document.getElementById("motd").innerText = "motd not available"
            }
